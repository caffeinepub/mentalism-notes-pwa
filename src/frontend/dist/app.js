...
function formatDate(d) {
  ...
  return `${d.getDate()} ${months[d.getMonth()]} ${d.getFullYear()} at ${h}:${mm}${ampm}`;
}

noteDateEl.textContent = formatDate(new Date());

// ── Note persistence (contenteditable) ───────────────────────
noteArea.textContent = state.note;

noteArea.addEventListener('input', () => {
  state.note = noteArea.textContent || '';
  saveData();
});
...
function onScreenPointer(e) {
  // Ignore taps inside settings panel or toolbar
  if (settingsOverlay.contains(e.target)) return;
  if (e.target.closest('#bottombar')) return;
...
function commitSelection() {
  ...
  const current = noteArea.textContent || '';
  const replaced = current.replace(/"([^"]*)"/,  '"' + chosen + '"');
  noteArea.textContent = replaced;
  state.note = replaced;
  saveData();
